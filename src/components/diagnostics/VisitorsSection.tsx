  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-3">
        <Stat label="بازدید امروز" value={formatStatNumber(v.today || 0)} color="text-emerald-400" card />
        <Stat label="میانگین 30 روز" value={formatStatNumber(v.avgDaily || 0)} color="text-cyan-400" card />
        <Stat label="حداکثر روزانه" value={formatStatNumber(v.maxDaily || 0)} color="text-violet-400" card />
        <Stat label="بازدیدکنندگان 7 روز اخیر" value={formatStatNumber(v.recent7d || 0)} color="text-amber-400" card />
        <Stat label="درصد بازگشتی (7 روز)" value={`${v.returnRate7d || 0}٪`} color="text-pink-400" card />
      </div>

      <Card title="بازدید 30 روز اخیر" icon={<TrendingUp className="h-4 w-4 text-emerald-400" />}>
        {daily.length === 0 ? (
          ...
          <div className="space-y-1.5">
            {daily.map((d: any) => (
              <div key={d.date} className="flex items-center gap-2 text-[10px]">
                ...
                <span className="w-10 text-slate-300 font-mono text-left">{formatStatNumber(d.count)}</span>
              </div>
            ))}
          </div>
        )}
            ...
            <div className="space-y-2">
              ...
              {hourly.map((h: any) => (
                <div key={h.hour} className="flex items-center gap-2 text-[11px]">
                  <span className="w-20 font-mono font-bold text-slate-200" dir="ltr">{formatStatNumber(h.hour)}:00</span>
                  <div className="flex-1 h-2.5 bg-slate-800/60 rounded-full overflow-hidden">
                    <div className="h-full bg-gradient-to-r from-cyan-500 to-blue-400" style={{ width: `${Math.max(3, (h.count / maxHour) * 100)}%` }} />
                  </div>
                  <span className="w-14 text-center font-mono font-bold text-cyan-300">{formatStatNumber(h.count)}</span>
                </div>
              ))}
              <div className="pt-1.5 text-[10px] text-slate-500">بازدیدهای 7 روز اخیر به تفکیک ساعت (0 تا 23)</div>
            </div>
          )}
        </Card>
            ...
            <div className="space-y-2">
              {v.byWeekday.map((w: any, i: number) => (
                <div key={i} className="flex items-center gap-2 text-[10px]">
                  ...
                  <span className="w-8 text-slate-300 font-mono text-left">{formatStatNumber(w.count)}</span>
                </div>
              ))}
            </div>
          ) : (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card title="محبوب‌ترین صفحات (30 روز)" icon={<FileText className="h-4 w-4 text-amber-400" />}>
          {pages.length === 0 ? (
            <div className="text-[11px] text-slate-500 py-3">داده‌ای ثبت نشده است.</div>
          ) : (
            <div className="space-y-2">
              {pages.map((p: any) => (
                <div key={p.path} className="flex items-center gap-2 text-[10px]">
                  ...
                  <span className="w-8 text-slate-200 font-mono text-left">{formatStatNumber(p.count)}</span>
                </div>
              ))}
            </div>
          )}
        ...
        <Card title="نمای کلی" icon={<Users className="h-4 w-4 text-pink-400" />}>
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-black/15 rounded-xl p-3 space-y-1.5">
              ...
              <div className="font-mono font-black text-lg text-pink-400">{formatStatNumber(v.total || 0)}</div>
            </div>
            <div className="bg-black/15 rounded-xl p-3 space-y-1.5">
              <Bot className="h-4 w-4 text-slate-400" />
              <div className="text-[9px] text-slate-500">ردشده به‌عنوان بات</div>
              <div className="font-mono font-black text-lg text-slate-300">{formatStatNumber(v.botsBlocked || 0)}</div>
            </div>
          </div>
          <div className="mt-3 p-3 rounded-xl bg-black/20 text-[10px] text-slate-500 leading-relaxed">
            کاربر برای شناسایی در مرورگر، یک شناسه یکتا ذخیره می‌شود و هر بازدید با آن شمارش می‌گردد؛ بازدیدهای تکراری هر صفحه در بازه 3 ثانیه نادیده گرفته می‌شود.
          </div>
          {v.lastVisit && (
